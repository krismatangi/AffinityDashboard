...
class AffinityDashboard {
    ...
    async loadConfiguration() {
        try {
            ...
            this.config = {
                ...
                mri_rate: 8, ct_rate: 12, xray_rate: 20, nucmed_rate: 6,
    ...
    async loadCaseData() {
        try {
            ...
            this.currentCases = {
                mri: data.mri_cases || 0,
                ct: data.ct_cases || 0,
                xray: data.xray_cases || 0,
                nucmed: data.nucmed_cases || 0
            };
    ...
    getCaseValues() {
        return this.currentCases || { mri: 0, ct: 0, xray: 0, nucmed: 0 };
    }
    ...
    calculateOverallStatus(cases) {
        const total = cases.mri + cases.ct + cases.xray + cases.nucmed;
        
        // Weighted scoring based on complexity (inverse system where 5 is normal, 1 is critical)
        const weightedScore = (cases.mri * 3) + (cases.ct * 2) + (cases.xray * 1) + (cases.nucmed * 4);
    ...
    updateSummaryStats(cases) {
        ...
        const maxTurnaround = Math.max(mriTurnaround, ctTurnaround, xrayTurnaround, nucmedTurnaround);
        const avgTurnaround = total > 0 ? Math.round((mriTurnaround + ctTurnaround + xrayTurnaround + nucmedTurnaround) / 4) : 0;
